...
const copy = {
  ...
  enquiryType: {
    ...
    options: {
      general: "አጠቃላይ ጥያቄ",
      product: "የምርት / ትዕዛዝ ጥያቄ",
      distributor: "አከፋፋይ ለመሆን",
      other: "ሌላ",
    },
  },
},
      submit: "መልዕክት ላክ",
      submitting: "በመላክ ላይ...",
      success: "እናመሰግናለን! መልዕክትዎ ደርሶናል። ጉዳዩ አስቸኳይ ከሆነ እባክዎ ከላይ በተጠቀሱት ስልክ ቁጥሮች ደውለው ያነጋግሩን።",
      failure: "ይቅርታ፣ መልዕክትዎ አልተላከም። እባክዎ እንደገና ይሞክሩ ወይም ከላይ በተጠቀሱት ስልክ ቁጥሮች ደውለው ያነጋግሩን።",
      errors: {
...
function Contact() {
  ...
  const contactSchema = z.object({
    ...
    message: z
      .string()
      .trim()
      .min(10, c.form.errors.messageMin)
      .max(1000, c.form.errors.messageMax),
  ...
  <label
    ...
    className="block text-[0.68rem] font-extrabold uppercase tracking-[0.18em] text-foreground"
  >
    {c.form.fields.enquiryType.label}
  </label>
  ...
  >
    <option value="general">{c.form.fields.enquiryType.options.general}</option>
    <option value="product">{c.form.fields.enquiryType.options.product}</option>
    <option value="distributor">{c.form.fields.enquiryType.options.distributor}</option>
